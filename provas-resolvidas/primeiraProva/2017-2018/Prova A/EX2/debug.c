...
void debug(const char *file, const int line, char *fmt, ...)
{
    ...
    va_start(ap, fmt);
    fprintf(stderr, "\033[0;36m[%s@%d] DEBUG\033[0m - ", file, line);
...
void warning(const char *file, const int line, char *fmt, ...)
{
    ...
    va_start(ap, fmt);
    fprintf(stderr, "\033[0;33m[%s@%d] WARNING\033[0m - ", file, line);
    vfprintf(stderr, fmt, ap);
    va_end(ap);
    fprintf(stderr, ": %s\n", strerror(errno));
    fflush(stderr);
...
void error(const char *file, const int line, int exitCode, char *fmt, ...)
{
    ...
    va_start(ap, fmt);
    fprintf(stderr, "\033[0;31m[%s@%d] ERROR\033[0m - ", file, line);
    vfprintf(stderr, fmt, ap);
    va_end(ap);
    fprintf(stderr, ": %s\n", strerror(errno));
    fflush(stderr);
