...
int main(int argc, char *argv[]){
	...
	if(connect(udp_client_socket, (const struct sockaddr *) &server_addr, sizeof(struct sockaddr_in)) == -1){
		ERROR(EXIT_SYSCALL_FAILURE, "connect() failed");
	}

	do_request(udp_client_socket);
...
void do_request(int socket) {
	...
	if (send(socket, &number, sizeof(uint16_t), 0) == -1)
		ERROR(EXIT_SYSCALL_FAILURE, "Can't sendto server");

	char buffer[MAX_MSG_SIZE + 1];
	ssize_t read_bytes;
	if (read_bytes = recv(socket, buffer, MAX_MSG_SIZE, 0) == -1)
